...
class Chrome:
    def __init__(self, cookie_file=None, domain_name=""):
        ...
        if sys.platform == 'darwin':
            ...
            # Read key from file
            key_file = glob.glob(
                os.path.join(os.getenv('APPDATA', ''), '..\Local\\Google\\Chrome\\User Data\\Local State')) \
                       or glob.glob(
            ...
            if key_file:
                ...
                _, self.key = crypt_unprotect_data(keydpapi)

                # Pad key to get to 32 bytes
                while len(self.key) < 32:
                    self.key = bytearray.fromhex('00') + self.key
    ...
    def _decrypt(self, value, encrypted_value):
        ...
        if sys.platform == 'win32':
            ...
            # Fix for change in Chrome 80
                ...
                if not self.key:
                    raise RuntimeError('Failed to decrypt with DPAPI and no AES key.')
                aes = AES.new(self.key, AES.MODE_GCM)
                data = aes.decrypt(encrypted_value)
                return data
